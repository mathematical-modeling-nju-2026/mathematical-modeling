...
paper/figure_package/（配图）、CUMCM2026Problems/C题/问题一/（问题一参考实现）。
"""
import pathlib
import sys
...
# (相对仓库路径, 展示标题前缀, 排序权重)
# 注意：不含各问 efficiency/ 下的「效率口径对照」代码（def2 敏感性分析专用，
...
GROUPS = [
    ...
    ('问题三：预报融合与场景树多时点调整', 3, [
        ...
        'question3/code/verify_q3.py', 'question3/code/plot_q3.py',
    ]),
    ('问题四-2：波动电价下的日前调度', 4, [
    ...
    ('问题四-3：波动电价下的多时点调整', 5, [
        ...
        'question4/part3/code/verify_q3.py', 'question4/part3/code/plot_q3.py',
    ]),
    ('公共模块：第二问基础数据与 LP（q2_base）', 6, [
        'common/q2_base/q2_data.py', 'common/q2_base/run_1439.py',
        'common/q2_base/make_figures_B.py',
    ]),
    ('公共模块：附件5结果模板表头（主程序依赖）', 7, [
        'common/efficiency/template_layout.py',
    ]),
    ('公共模块：绘图（plotting）', 8, [
        'common/plotting/export_figure.py', 'common/plotting/setup_style.py',
    ]),
    ('论文配图生成', 9, [
        'paper/figure_package/build_figures.py',
        'paper/figure_package/verify_package.py',
    ]),
    ('问题一参考实现（题目提供的工作目录）', 10, [
        'CUMCM2026Problems/C题/问题一/solve_q1.py',
        'CUMCM2026Problems/C题/问题一/verify_q1.py',
        'CUMCM2026Problems/C题/问题一/make_result1.py',
        'CUMCM2026Problems/C题/问题一/make_figures.py',
        'CUMCM2026Problems/C题/问题一/diagnose_surplus.py',
    ]),
]
...
lines.append('')
lines.append('以下按问题列出建模用到的全部源程序，代码与支撑材料 \\texttt{support.zip} 中')
lines.append('同名文件\\textbf{逐字一致}；运行环境与复现命令见 \\S\\ref{sec:appendix-env}。')
