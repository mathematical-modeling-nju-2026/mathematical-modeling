"""
C题 问题1：确定性储能调度优化模型（LP / MILP）—— 参数化效率版本

本文件由 question1/code/q1_model.py 参数化而来，唯一改动是
把原写死的单向效率 ETA 拆成充电侧 ETA_C 与放电侧 ETA_D 两个参数，
以便在同一套代码下对比「题目 90% 效率」的两种国标/储能口径。

两种口径
--------
定义一（储能/逆变器侧，本文原口径）：
    「充放电效率均为 90%」理解为
        充电侧  E_t = E_{t-1} + η·c_t      （存进电池的比例 = 90%）
        放电侧  E_t = E_{t-1} - d_t/η      （放出外部所需的比例 = 90%）
    即 η_c = η_d = 0.90，往返效率 η_c·η_d = 0.81。
    物理含义：充入 1 kWh，电池内只增加 0.9 kWh；电池内取 1 kWh，外部只得 0.9 kWh。

定义二（GB/T 34131 系国标口径，放电量/充电量）：
    「充放电效率为 90%」理解为**放电量 / 充电量 = 90%**，
    即往返（round-trip）效率 = 0.90。
    按对称拆分 η_c = η_d = sqrt(0.90) ≈ 0.948683，
    使 η_c·η_d = 0.90 且两侧损耗相同。

其他口径（供参考，本文件不采用）：
    仅充电侧损耗 η_c=0.9, η_d=1.0；仅放电侧损耗 η_c=1.0, η_d=0.9。
    二者往返效率同为 0.9，但充/放电量统计口径不对称。
...
# ---------------- 效率定义（本文件的核心参数） ----------------
EFF_DEFS = {
    # key: (eta_c, eta_d, 往返效率, 说明)
    "def1_side90": (
        0.9, 0.9, 0.9 * 0.9,
        "定义一：充/放效率各 90%（储电量/充电量 = 放电量/储电量 = 0.9），往返 0.81"
    ),
    "def2_roundtrip90": (
        np.sqrt(0.9), np.sqrt(0.9), 0.9,
        "定义二：放电量/充电量 = 90%（国标往返口径），对称拆分 sqrt(0.9)≈0.948683"
    ),
}

# 默认口径 = 定义一（与仓库原有结果一致）
EFF_KEY = "def1_side90"
ETA_C, ETA_D, ETA_RT, EFF_DESC = EFF_DEFS[EFF_KEY]


def set_efficiency(key):
    """切换效率口径。key ∈ EFF_DEFS。返回 (eta_c, eta_d, 往返)。"""
    global EFF_KEY, ETA_C, ETA_D, ETA_RT, EFF_DESC
    if key not in EFF_DEFS:
        raise KeyError(f"未知效率口径 {key!r}；可选 {list(EFF_DEFS)}")
    EFF_KEY = key
    ETA_C, ETA_D, ETA_RT, EFF_DESC = EFF_DEFS[key]
    return ETA_C, ETA_D, ETA_RT


def efficiency_info():
    """返回当前口径的说明元组 (key, eta_c, eta_d, 往返, 描述)。"""
    return EFF_KEY, ETA_C, ETA_D, ETA_RT, EFF_DESC
...
def build_lp(price, load, pv):
    ...
    # ② 电量递推：E_t = E_{t-1} + η_c·c_t - d_t/η_d
    for t in range(T):
        r = T + t
        A_eq[r, E.start + t] = 1.0
        if t > 0:
            A_eq[r, E.start + t - 1] = -1.0
        A_eq[r, C.start + t] = -ETA_C
        A_eq[r, D.start + t] = 1.0 / ETA_D
...
def solve_milp(price, load, pv):
    ...
    # ② 电量递推
    for t in range(T):
        coefs = [(E.start + t, 1.0), (C.start + t, -ETA_C),
                 (D.start + t, 1.0 / ETA_D)]
        if t > 0:
...
def verify(sol, price, load, pv, tol=1e-6):
    ...
    # 电量递推一致性（独立重算）
    E_rec = E_INIT + np.cumsum(ETA_C * c - d / ETA_D)
