...
融合预报的精度提升集中在日间：6--12 时段 MAE 由 $309.24$ kW 降至 $169.41$ kW
（降幅约 $45\%$），12--18 时段由 $288.99$ kW 降至 $133.68$ kW（降幅约 $54\%$）。
这正是光伏出力大、预报误差绝对值高的时段，也是购电决策对误差最敏感的时段，
因此融合预报的主要价值来自\textbf{在误差最大的时段压降误差}。夜间绝对误差基数小，
改善空间有限，图中单独展示仅为保留其变化尺度，并不改变上述判断。
进一步看，融合权重随历史样本的成熟而更新、并叠加偏差校正，
说明融合输出并非两条预报的固定加权平均，而是随数据逐步修正的动态估计。
需要强调，\textbf{MAE 的下降并不能机械地换算成等比例的费用下降}：
前者是统计精度指标，后者还取决于误差落在哪些时段、以及储能能否跨时段吸收这些偏差，
故精度结论须与图 \ref{fig:q3-info-value} 的费用对照结合判断。
...
图 \ref{fig:q3-adjust} 揭示了一个容易被忽略的事实：日内调整对最终购电量的
\textbf{修正幅度其实很小}。若把调整后的曲线与原计划直接叠加，两条几乎重合的曲线
会掩盖这一修正；只有以原计划为零基准，才能看清各执行区间内增购与减购的方向与量级。
这说明问题三的收益并非来自大幅改计划，而是来自\textbf{对原计划做小幅、及时的纠偏}。
储电量轨迹按跨日滚动衔接，指定日的首末电量并不要求相等，
体现了“日末价值留给次日”的滚动视角。图中紧急购电标记仅用于指示发生时段，
既不代表缺口大小，也不能仅凭它与储能状态的相邻关系推断因果——
实际缺口的形成是计划、储能与预报误差共同作用的结果。
