...
\noindent\textbf{举例}：原计划 $100\kw$、实际取消 $20\kw$，电价 $1$ 元/kWh。
...
\noindent 若把整点值当作“小时均值”作分段常数填充，会引入
...
\noindent\textbf{跨日前瞻}：每次优化至\textbf{次日 24 点}，把次日作为确定性价值近似，
避免“只顾当天便宜电价而提前耗空电池”。
不同未来节点使用其条件预报修订后的次日点预测。

\noindent\textbf{终端目标}：一般日期前瞻终点回到\textbf{本日 0 点电量}；
且 $6/12/18$ 点重算时该目标\textbf{保持不变}（不随当前电量漂移），
否则会引入“越调整越保守”的路径依赖。

\noindent\textbf{贪心执行}：只执行当前发布到下一允许更新前的控制量：
...
\noindent\textbf{为何不把问题二的 $80\%$ 分位套用到问题三？}
...
\noindent 相对同口径方案 B，主方案\textbf{节省 $367\,495.52$ 元（$2.55\%$）}，
...
\noindent\textbf{结论}：$6$ 点与 $12$ 点各贡献约 $8$ 万元，收益随调整次数\textbf{显著边际递减}；
...
\noindent\textbf{自适应融合的精度已优于“含泄漏的静态最优”}，
...
\noindent 可见：\textbf{仅“场景缩放系数 $k$”一个维度有正收益}（约 $0.47$ 万元，$0.03\%$），
...
\noindent 四个指定日期的净调整费\textbf{均为负}，即调整机制在指定日期上均实现了“省违约金”的净收益。
