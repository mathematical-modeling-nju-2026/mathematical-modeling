...
在写法上，本小节先说本问\textbf{必须满足的信息约束}，再给模型，
因为该约束决定了模型可以取什么形式。问题二已要求“预测只能用历史数据”，
...
这一疑问应在建模之前回答，因为它决定了本问不应假设一个固定分位。
问题二的 $F^*=0.80$ 是在“储能固定、单期补救费为 $5$ 倍、无后续调整”
这三个条件下导出的单期临界分位；而问题三中 0 点可以从场景树
\textbf{评估未来的调整机会}，即今天的“少买”可以在 6/12/18 点以 $1.5$ 倍价补上，
故有效的风险水平随节点、时段而变，
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{04_q3_information_value.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{05_q3_forecast_fusion.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{06_q3_rolling_adjustment.pdf}
...
也不能据其与储能状态的相邻关系断言单一原因。原曲线图
（\texttt{q3\_cost.pdf}、\texttt{q3\_dispatch.pdf}）保留作补充展示。
