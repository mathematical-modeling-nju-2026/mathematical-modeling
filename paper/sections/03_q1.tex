...
把优化结果与无储能基线逐项对比。由逐时段功率平衡
$g_t=L_t+c_t-d_t-V_t+w_t$（基线取 $c=d=0$ 且 $w_t=\max(V_t-L_t,0)$）
两式相减得 $g_t^{\mathrm{base}}-g_t=\max(V_t-L_t,0)-w_t+d_t-c_t$，
逐段乘以 $p_t$ 并求和，可得\textbf{严格恒等式}
\begin{equation}
\sum_t p_t\max(L_t-V_t,0)-\sum_t p_tg_t
=\underbrace{\sum_t p_t\bigl[\max(V_t-L_t,0)-w_t\bigr]}_{\text{弃光消除}}
+\underbrace{\sum_t p_td_t-\sum_t p_tc_t}_{\text{净套利}},
\label{eq:q1-decomp}
\end{equation}
即“总节省 $=$ 弃光消除 $+$ 净套利”，两项之和恒等于总节省，无重复、无遗漏。
本实例优化解中 $w_t\equiv0$（全天弃光为零），故弃光消除项即
$\sum_t p_t\max(V_t-L_t,0)=4037.75$ 元；一般情形下若存在弃光，
须按式 \eqref{eq:q1-decomp} 扣除 $\sum_t p_tw_t$ 后等式才严格成立。
...
\textbf{套利是主要来源（约 69\%）}：本例光伏余电仅 $6247.96\kw$，
而峰谷价差最高 $1.40/0.37\approx 3.8$ 倍，远超往返盈亏平衡点 $1/0.81=1.235$，
故储能边际价值主要在跨时段搬移电量而非吸收余电。
...
\textbf{为何弃光为 0：}光伏总量 $55\,482.84\kw$ 全部被利用（部分直接供负载、部分存入电池），
说明本日电池容量足以吸收全部余电，储能边际价值主要在套利而非消纳。

\textbf{为何充放电发生在这些时段：}附件 1 电价呈\textbf{三段特征}：
凌晨 $0$--$6$ 时低谷（$0.37$--$0.44$ 元/kWh，大量充电）、
上午 $6$--$9$ 时尖峰（最高 $1.01$，放电供负载）、
中午回落（$0.41$--$0.53$，光伏余电 $+$ 补充电）、
傍晚 $18$--$20$ 时全天最高（$1.35$--$1.40$，放电）。
调度逻辑即在凌晨与中午低价窗口充电、在上午与傍晚高价窗口放电，最大化价差收益。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{01_q1_dispatch.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{02_q1_savings.pdf}
...
这一分解是逐时段购电费差额的恒等核算，与表 \ref{tab:q1-gain} 按能量来源的
$4037.75$ 元与 $8887.35$ 元分解各自自洽，不互相替代，也不把同一节省重复归因。
