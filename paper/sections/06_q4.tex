...
\begin{tabular}{@{}l r r r@{}}
\toprule
电价情形 & {仅 0 点决策（元）} & {$0/6/12/18$ 点（元）} & 表观差值（元） \\
\midrule
...
需要说明：上表两行\textbf{口径并不对称}——固定电价一行的左侧是问题二的最终方案，
右侧问题三除多时点调整外还额外引入了\textbf{在线预报融合}，
因此 $366\,738$ 元中包含融合预报的贡献，不能直接与波动电价一行的
$101\,308$ 元比较。

为在\textbf{同等口径}下分离出“多时点调整”本身的边际价值，需要剔除融合预报的贡献。
对固定电价情形，问题三 \S\ref{sec:q3-sens} 给出了逐级增量：
“无融合的 0 点方案 $\to$ 融合 0 点方案”节省 $208\,447.47$ 元（融合的价值），
“融合 0 点方案 $\to$ 0/6/12/18 点多时点方案”节省 \textbf{$159\,048.05$ 元}（调整的价值）。
故固定电价下\textbf{纯调整机制}的价值为 \textbf{约 $15.9$ 万元}。
...
电价情形 & {调整机制边际价值（万元）} & 依据 \\
\midrule
固定电价   & $15.9$  & 问题三：融合 0 点 $\to$ 0/6/12/18 点 \\
\textbf{波动电价} & \bfseries $10.1$ & 4-2 $\to$ 4-3 的总增量 \\
\bottomrule
\end{tabular}
\end{table}

由该表可见，\textbf{在同等口径下，波动电价并未放大调整机制的价值}：
固定电价下为 $15.9$ 万元，波动电价下反而降为 $10.1$ 万元（约为前者的 $64\%$）。
这与“电价波动会放大调整收益”的直觉相反，原因有二：
(i) 4-3 的电价在每次发布时刻只能用\textbf{此前 7 天同时段均值}预测，
多时点更新对\textbf{电价预期}的修正能力有限，新增信息主要来自光伏预报；
(ii) 4-2 的 0 点计划已采用电价随机场景，本身已对电价不确定性作了较好的预备，
留给后续调整的改进空间因此收窄。
换言之，\textbf{电价不确定性主要被 0 点的随机规划吸收，而非被多时点调整吸收}。
