...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{07_q42_price_information.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{08_q42_schedule_shift.pdf}
...
\textbf{在完全同口径下，波动电价显著放大了多时点调整的价值}：
由 $15.90$ 万元增至 $23.34$ 万元（高约 $47\%$），
而融合预报的价值仅由 $20.84$ 万元增至 $22.19$ 万元。
边际递减形态也印证了这一点：固定电价下第三次调整（加 $18$ 点）已零收益
（$-38.88$ 元），波动电价下仍有 $+6\,875$ 元——电价波动\textbf{推迟了}调整机制的饱和点，
因为多时点更新不仅能修正光伏预报，还能修正\textbf{电价预期}。

需强调：$4\text{-}2 \to 4\text{-}3$ 的主方案差值为 $101\,308.46$ 元，
但该差值横跨两问、同时含“预测源切换”与“引入调整机制”两种效应，
\textbf{不能}作为调整机制的边际价值；上表的逐级增量才是同口径度量。
两问主方案绝对费用仍相差约 $104$ 万元，主要来自附件 4 的实时电价整体高于附件 1。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{09_q43_incremental_comparison.pdf}
...
其中紧急费差额 $877\,139.21$ 元占总差额的 $84.39\%$；
该差异是不同电价输入及对应滚动策略共同形成的账面差异，不能全部解释为因果成本。
各问内部的逐级对照中，加 6 点节省由 $8.01$ 万元变为 $12.20$ 万元、
加 12 点由 $7.90$ 万元变为 $10.45$ 万元、加 18 点由 $-38.88$ 元变为 $+6\,875.04$ 元，
表明新增调整机会的回测价值随运行情形而变化。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{10_q43_schedule_differences.pdf}
...
各指定日初始储电量已由此前滚动执行决定、两问并不相同，
故状态差异既含当日决策差异、也含前期累积影响，
不将某一列的颜色直接解释为另一列的因果效应。
