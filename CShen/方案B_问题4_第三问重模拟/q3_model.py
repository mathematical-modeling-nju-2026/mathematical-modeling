...
def solve(forecaster, day, stage, stages, energy, day_start_energy,
    ...
    price = forecaster.price_point(day, stage) if hasattr(forecaster, 'price_point') else forecaster.data['price']
    ...
    S = len(scenarios)
    price_scenarios, price_past = (forecaster.price_scenarios(day, stage) if hasattr(forecaster, 'price_scenarios')
                                   else (np.broadcast_to(price, (S, T)), past))
    if len(price_past) != len(past):
        raise RuntimeError('Price and net-load scenario samples must use the same historical days.')
    lp = LP()
    ...
    for ni, node in enumerate(nodes):
        ...
        node_price = np.mean(price_scenarios[node.members, start:stop], axis=0)
        ...
        for j, t in enumerate(range(start, stop)):
            ...
            lp.le([(node.q[j], -1), (node.d[j], -1), (node.c[j], 1)], -node_point[t])
        emergency = lp.var(len(node.members) * n, (5 * price_scenarios[node.members, start:stop] / S).ravel())
