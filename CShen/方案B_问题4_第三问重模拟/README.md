...
采用“在线预报融合 + 经验场景树LP + 跨日滚动执行”，继承方案B的负载与历史光伏预测内核。数据直接读取 C 题附件1—3，不依赖 `C_yang/raw`；可通过环境变量 `CUMCM_C_ATTACHMENT_DIR` 指定附件目录。

## 运行

在本目录执行（复用方案B已创建的虚拟环境）：

```powershell
& "..\方案B_1439万\.venv\Scripts\python.exe" -m pip install -r .\requirements.txt
& "..\方案B_1439万\.venv\Scripts\python.exe" -X utf8 .\run_q3.py --mode all
& "..\方案B_1439万\.venv\Scripts\python.exe" -X utf8 .\verify_q3.py
& "..\方案B_1439万\.venv\Scripts\python.exe" -X utf8 .\plot_q3.py
```
